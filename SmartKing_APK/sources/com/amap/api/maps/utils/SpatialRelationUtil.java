...
public class SpatialRelationUtil {
    ...
    public static Pair<Integer, DPoint> calShortestDistancePoint(List<DPoint> list, DPoint dPoint, float f) {
        ...
        while (true) {
            ...
            if (!checkRotateIsMatch(dPoint4, dPoint5, f)) {
                i = i3;
            } else {
                ...
                Pair<Double, DPoint> pointToSegDist = pointToSegDist(dPoint2.x, dPoint2.y, dPoint4.x, dPoint4.y, dPoint5.x, dPoint5.y);
                if (pair2 == null) {
    ...
    private static boolean checkRotateIsMatch(DPoint dPoint, DPoint dPoint2, float f) {
        ...
        float abs = Math.abs((fr.a(dPoint, dPoint2) + 360.0f) - f) % 360.0f;
        if (abs > 180.0f) {
            abs = 360.0f - abs;
        }
        return abs < 50.0f;
