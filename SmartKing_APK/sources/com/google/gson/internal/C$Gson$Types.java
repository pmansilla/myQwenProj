    ...
    private static Type resolve(Type type, Class<?> cls, Type type2, Collection<TypeVariable> collection) {
        ...
        if (!(type2 instanceof ParameterizedType)) {
            ...
            if (lowerBounds.length == 1) {
                Type resolve3 = resolve(type, cls, lowerBounds[0], collection);
                if (resolve3 != lowerBounds[0]) {
                    return supertypeOf(resolve3);
                }
            } else if (upperBounds.length == 1) {
                Type resolve4 = resolve(type, cls, upperBounds[0], collection);
        ...
        return z ? newParameterizedTypeWithOwner(resolve5, parameterizedType.getRawType(), actualTypeArguments) : parameterizedType;
    }
