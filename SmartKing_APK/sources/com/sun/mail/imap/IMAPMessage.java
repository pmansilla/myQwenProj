...
public class IMAPMessage extends MimeMessage implements ReadableMime {
    ...
    public String getFileName() throws MessagingException {
        ...
        loadBODYSTRUCTURE();
        String str = this.bs.dParams != null ? this.bs.dParams.get(FileDownloadModel.FILENAME) : null;
        ...
        synchronized (getMessageCacheLock()) {
            try {
                try {
                    ...
                    checkExpunged();
                    if (protocol.isREV1()) {
                        BODY peekBody = protocol.peekBody(getSequenceNumber(), toSection("HEADER.FIELDS (" + str + SQLBuilder.PARENTHESES_RIGHT));
                        if (peekBody != null) {
                            byteArrayInputStream = peekBody.getByteArrayInputStream();
                        }
                        byteArrayInputStream = null;
                    } else {
                        RFC822DATA fetchRFC822 = protocol.fetchRFC822(getSequenceNumber(), "HEADER.LINES (" + str + SQLBuilder.PARENTHESES_RIGHT);
                        if (fetchRFC822 != null) {
                            byteArrayInputStream = fetchRFC822.getByteArrayInputStream();
                        }
                        byteArrayInputStream = null;
    ...
    public String getInReplyTo() throws MessagingException {
        checkExpunged();
        if (this.bodyLoaded) {
            return super.getHeader("In-Reply-To", SQLBuilder.BLANK);
        }
        loadEnvelope();
    ...
    public Address[] getReplyTo() throws MessagingException {
        ...
        loadEnvelope();
        return (this.envelope.replyTo == null || this.envelope.replyTo.length == 0) ? getFrom() : aaclone(this.envelope.replyTo);
    }
