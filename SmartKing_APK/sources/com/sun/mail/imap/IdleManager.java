...
public class IdleManager {
    ...
    private static String folderName(Folder folder) {
        try {
            ...
            return folder.getStore().toString() + FileUriModel.SCHEME + folder.toString();
        }
    }
    ...
    public void select() {
        ...
        while (!this.die) {
            try {
                ...
                this.logger.log(Level.FINEST, "IdleManager interrupted", (Throwable) e4);
                this.die = true;
                this.logger.finest("IdleManager unwatchAll");
                try {
                    unwatchAll();
                ...
                this.logger.log(Level.FINEST, "IdleManager got I/O exception", (Throwable) e6);
                this.die = true;
                this.logger.finest("IdleManager unwatchAll");
                try {
                    unwatchAll();
        ...
        while (true) {
            ...
            if (channel != null) {
                try {
                    ...
                    this.logger.log(Level.FINEST, "IdleManager exception while aborting idle for folder: " + folderName(poll), (Throwable) e2);
                }
            }
        }
    }
    ...
    public void watch(Folder folder) throws MessagingException {
        ...
        if (this.logger.isLoggable(Level.FINEST)) {
            if (i > 0) {
                this.logger.log(Level.FINEST, "IdleManager.watch startIdle succeeded for {0} after " + i + " tries", folderName(iMAPFolder));
            } else {
