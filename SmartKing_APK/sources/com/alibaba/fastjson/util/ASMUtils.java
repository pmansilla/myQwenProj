...
public class ASMUtils {
    ...
    public static String[] lookupParameterNames(AccessibleObject accessibleObject) {
        ...
        InputStream resourceAsStream = classLoader.getResourceAsStream(declaringClass.getName().replace(ClassUtils.PACKAGE_SEPARATOR_CHAR, '/') + ".class");
        try {
    ...
    public static String type(Class<?> cls) {
        if (!cls.isArray()) {
            return !cls.isPrimitive() ? cls.getName().replace(ClassUtils.PACKAGE_SEPARATOR_CHAR, '/') : getPrimitiveLetter(cls);
        }
        return "[" + desc(cls.getComponentType());
    }
}
