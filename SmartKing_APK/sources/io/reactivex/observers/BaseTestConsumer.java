...
public abstract class BaseTestConsumer<T, U extends BaseTestConsumer<T, U>> implements Disposable {
    ...
    public final U assertValueSequence(Iterable<? extends T> iterable) {
        ...
        if (hasNext) {
            throw fail("More values received than expected (" + i + SQLBuilder.PARENTHESES_RIGHT);
        }
        if (!hasNext2) {
            return this;
        }
        throw fail("Fever values received than expected (" + i + SQLBuilder.PARENTHESES_RIGHT);
    }
    ...
    public final U assertValues(T... tArr) {
        int size = this.values.size();
        if (size != tArr.length) {
            throw fail("Value count differs; Expected: " + tArr.length + SQLBuilder.BLANK + Arrays.toString(tArr) + ", Actual: " + size + SQLBuilder.BLANK + this.values);
        }
        for (int i = 0; i < size; i++) {
