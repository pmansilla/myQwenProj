...
public abstract class Single<T> implements SingleSource<T> {
    ...
    public static <T> Flowable<T> concat(Publisher<? extends SingleSource<? extends T>> publisher, int i) {
        ...
        return RxJavaPlugins.onAssembly(new FlowableConcatMap(publisher, SingleInternalHelper.toFlowable(), i, ErrorMode.IMMEDIATE));
    }
    ...
    public static <T> Flowable<T> merge(Publisher<? extends SingleSource<? extends T>> publisher) {
        return RxJavaPlugins.onAssembly(new FlowableFlatMap(publisher, SingleInternalHelper.toFlowable(), false, Integer.MAX_VALUE, Flowable.bufferSize()));
    }
    ...
    public static <T, R> Single<R> zipArray(Function<? super Object[], ? extends R> function, SingleSource<? extends T>... singleSourceArr) {
        ...
        for (SingleSource<? extends T> singleSource : singleSourceArr) {
            ObjectHelper.requireNonNull(singleSource, "The " + i + "th source is null");
            publisherArr[i] = RxJavaPlugins.onAssembly(new SingleToFlowable(singleSource));
            i++;
        }
